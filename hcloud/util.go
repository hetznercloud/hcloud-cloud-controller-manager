...
func getRobotServerByName(c robot.Client, node *corev1.Node) (server *robotmodels.Server, err error) {
	...
	if robot.IsRateLimitExceeded() {
		return nil, fmt.Errorf("%s: rate limit exceeded - next try at %q", op, robot.TimeOfNextPossibleAPICall().String())
	}

	serverList, err := c.ServerGetList()
	if err != nil {
		robot.HandleRateLimitExceededError(err)
		return nil, fmt.Errorf("%s: %w", op, err)
	}
...
func getRobotServerByID(c robot.Client, id int, node *corev1.Node) (*robotmodels.Server, error) {
	...
	if err != nil && !robotmodels.IsError(err, robotmodels.ErrorCodeServerNotFound) {
		robot.HandleRateLimitExceededError(err)
		return nil, fmt.Errorf("%s: %w", op, err)
	}
...
func getInstanceTypeOfRobotServer(server *robotmodels.Server) string {
	if server == nil {
...
func getZoneOfRobotServer(server *robotmodels.Server) string {
	return strings.ToLower(server.Dc)
}

func getRegionOfRobotServer(server *robotmodels.Server) string {
